...
import {
    AllJobDocument, maintenanceDocumentUpdateById, maintenanceDocumentUpdateByIdFresh, maintenanceDocumentDeleteById, maintenanceDocumentDeleteByIdFresh,
    AllDocumentByJob
} from "store/actions";
import moment from "moment";
import toastr from "toastr";

function JobAllDocument(props) {
    const { id } = useParams();
    ...
    const ownerRef = (cell, row) => {
        console.log(row);
        return <span className="text-primary" onClick={() => pushOwner(row?.property.owner_id)}>{row?.property?.owner_id ? row?.property?.owner : ''}</span>;
    };

    const tenantRef = (cell, row) => {
        return <span className="text-primary" onClick={() => pushTenant(row?.property?.tenant_id)}>{row?.property?.tenant_id ? row?.property?.tenant : ''}</span>;
    };
    ...
    const handleSubmit = (e) => {
        ...
        props.maintenanceDocumentUpdateById(rowData, docID);
        setModal(false)
    ...
    const handleSubmit1 = (e) => {
        e.preventDefault();
        //console.log(rowData);
        props.maintenanceDocumentDeleteById(docID);
        setModal1(false)
    ...
    console.log(props.all_document_by_job_data);
    ...
    useEffect(() => {
        if (init) {
            props.AllDocumentByJob(id, 'Uploaded');
            setInit(false)
        }
    ...
    const toggle = (tab, type) => {
        ...
        if (tab == 1) {
            ...
            props.AllDocumentByJob(id, 'Generated')
        }
    };
                                                ...
                                                <Row>
                                                    <Col sm="12">
                                                        <CardText className="mb-0">
                                                            {props.all_document_by_job_data?.data ? (
                                                                <DatatableTables2
                                                                    products={props.all_document_by_job_data.data}
...
const mapStateToProps = gstate => {
    const {
        ...
        maintenance_document_name_update_error,
        maintenance_document_name_update_loading,

        maintenance_document_delete_error,
        maintenance_document_delete_loading, all_document_by_job_data, all_document_by_job_loading
    } = gstate.Document;
    return {
        all_job_document,
        all_job_document_error,
...
export default withRouter(
    connect(mapStateToProps, {
        AllJobDocument,
        maintenanceDocumentUpdateById,
        maintenanceDocumentUpdateByIdFresh,
        maintenanceDocumentDeleteById,
        maintenanceDocumentDeleteByIdFresh, AllDocumentByJob
    })(JobAllDocument)
);
