...
import { supplierFolioLedgerReport, storeAttachmentForSendInEmail, FilterSupplierFolioLedgerReportApi, FilterSupplierFolioLedgerReportApiFresh } from "store/actions";
...
const SupplierFolioLedger = props => {
    ...
    useEffect(() => {
        props.supplierFolioLedgerReport(id);
    }, []);
    useEffect(() => {
        if (props.supplier_fl_data_loading === 'Success') {
            endLoader()
            props.FilterSupplierFolioLedgerReportApiFresh()
        }
    }, [props.supplier_fl_data_loading])

    return (
        <React.Fragment>
            <div>
                <InvoiceHeader printDiv={printDiv} downloadPdfDocument={downloadPdfDocument} sendDocumentInEmail={sendDocumentInEmail} />
                <SupplierFolioLedgerFilter startLoader={startLoader} id={id}
                    FilterSupplierFolioLedgerReportApi={props.FilterSupplierFolioLedgerReportApi}
                />
                                    ...
                                    <div>
                                        ...
                                        <div className="table-responsive">
                                            <Table className="table mb-0">
                                                ...
                                                <tbody>
                                                    {
                                                        props.supplier_fl_data?.data?.folio_ledger?.length > 0 &&
                                                        <tr>
                                                            <td colSpan="12">
                                                                <span className="fw-bold font-size-16">
                                                                    {props.supplier_fl_data?.data?.supplier_contacts?.reference} -{" "}
                                                                    {props.supplier_fl_data?.data?.folio_code}
                                                                </span>
                                                            </td>
                                                        </tr>
                                                    }
                                                    {props.supplier_fl_data?.data?.folio_ledger.map((item2, index2) => {
...
const mapStateToProps = gstate => {
    const { supplier_fl_data, supplier_fl_data_loading } = gstate.AccountsTransactions;
    const { mail_attachment_data } = gstate.Message;
    return {
        supplier_fl_data, supplier_fl_data_loading, mail_attachment_data
    };
};

export default withRouter(
    connect(mapStateToProps, {
        supplierFolioLedgerReport, storeAttachmentForSendInEmail, FilterSupplierFolioLedgerReportApi, FilterSupplierFolioLedgerReportApiFresh,SupplierFolioLedgerFilter
    })(SupplierFolioLedger)
);
