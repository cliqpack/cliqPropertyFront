...
import { AllTaskDocument, taskDocumentUpdateById, taskDocumentUpdateByIdFresh, taskDocumentDeleteById, taskDocumentDeleteByIdFresh } from "store/actions";
import moment from "moment";
import toastr from "toastr";

function TaskAllDocument(props) {
    const { id } = useParams();
    ...
    //     return <a href={process.env.REACT_APP_DOCUMENT + row.doc_path} target="_blank" rel="noreferrer noopener">{row.doc_path.slice(0, 35)}</a>;
    // };
    const fileRef = (cell, row) => {
    ...
    useEffect(() => {
        if (props.all_task_document_loading === false) {
            props.AllTaskDocument(id);
        }

        if (props.task_document_name_update_loading === "Success") {
            toastr.success('Edited Successfull');
            props.taskDocumentUpdateByIdFresh();
            props.AllTaskDocument(id);
        }

        if (props.task_document_delete_loading === "Success") {
            toastr.success('Deleted Successfull');
            props.taskDocumentDeleteByIdFresh();
            props.AllTaskDocument(id);
        }
    }, [props.all_task_document_loading, props.task_document_name_update_loading, props.task_document_delete_loading]);
                                                ...
                                                <Row>
                                                    <Col sm="12">
                                                        <CardText className="mb-0">
                                                            {props.all_task_document ? (
                                                                <DatatableTables2
                                                                    products={props.all_task_document.data}
...
const mapStateToProps = gstate => {
    const {
        ...
        task_document_name_update_error,
        task_document_name_update_loading,

        task_document_delete_error,
        task_document_delete_loading,
    } = gstate.Document;
    return {
        all_task_document,
        all_task_document_error,
...
export default withRouter(
    connect(mapStateToProps, {
        AllTaskDocument,
        taskDocumentUpdateById,
        taskDocumentUpdateByIdFresh,
        taskDocumentDeleteById,
        taskDocumentDeleteByIdFresh
    })(TaskAllDocument)
);
