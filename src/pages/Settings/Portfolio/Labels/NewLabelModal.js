...
import {
  companyDataForPortfolioLabelsAdd,
  companyDataForPortfolioLabelsAddFresh,
...
const NewLabelModal = props => {
  ...
  const handleSave = e => {
    e.preventDefault();
    props.companyDataForPortfolioLabelsAdd(state);
  };

  useEffect(() => {
    if (props.label_add_loading === "Success") {
      toastr.success("Success");

      props.companyDataForPortfolioLabelsAddFresh();
      props.companyDataForPortfolioLabel();
      props.toggle();
    }
  }, [props.label_add_loading]);
  ...
  return (
    <>
      <Modal
        isOpen={props.labelModal}
        toggle={props.toggle}
        scrollable={true}
        size="lg"
      >
        <ModalHeader toggle={props.toggle} className="text-info">New Label</ModalHeader>
          ...
          <Row>
            <Col lg={12}>
              ...
              <Row className="py-2">
                ...
                <Col md={8} className="">
                  <div className={`w-50 ${state.infoBtn ? 'text-info' : 'text-danger'}`}>{state.label_name}</div>
                </Col>
              </Row>
            </Col>
          </Row>
...
const mapStateToProps = gstate => {
  const { label_add_loading } = gstate.Portfolio;
