...
import {
    contactList, updatePropertyInfoFromListingList, updatePropertyInfoFromListingListFresh, ListingListInfo
} from "store/actions";
...
const EditPropertyPage = props => {
    ...
    // console.log(addressState.street?.length);
    ...
    console.log(props.property_edit_info_data?.data?.data?.location);
    ...
    useEffect(() => {
        ...
        if (props.property_update_listing_info_loading === "Success") {
            ...
            props.getPropertyInfo(p_id);
            props.getPropertyEditInfo(p_id);
            props.ListingListInfo(l_id)
            // props.propertyUpdateFresh();
            // history.push("/listingInfo/" + l_id);
            setStatus(false)
            history.push(`/listingInfo/${l_id}`)
        }

        if (props.property_update_listing_info_loading === "Failed") {
            toastr.error("Failed");
            props.updatePropertyInfoFromListingListFresh();
            setStatus(false)
        ...
        if (props.property_edit_info_data != undefined) {
            ...
            setEditPropertyState({
                ...
                routine_inspections_frequency_type:
                    props.property_edit_info_data?.data?.data?.routine_inspections_frequency_type,
        ...
        props, props.user_list_data, props.property_type_data
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if (editPropertyState.reference == undefined && editPropertyState.reference?.length == 0) {
            ...
            setAddressShow(true);
        } else if (addressState.suburb?.length == 0) {
            toastr.warning("Suburb cannot be empty");
            setAddressShow(true);
        } else if (addressState.number?.length == 0) {
            toastr.warning("Number cannot be empty");

        }
        else if (editPropertyState.bedroom == null || (Number(editPropertyState.bedroom)) == 0 ||
            editPropertyState.bathroom == null || (Number(editPropertyState.bathroom)) == 0
            ||
            editPropertyState.car_space == null || (Number(editPropertyState.car_space)) == 0) {
            toastr.warning('Bedroom, Bathroom and Card space cannot be 0')
        }
        else {
            // toastr.success('in')
            // return
            setStatus(true)
            props.updatePropertyInfoFromListingList(l_id, p_id, editPropertyState, addressState, state);
        }
    }

    const handleFloorSize = e => {
        setEditPropertyState({ ...editPropertyState, ['floorSize']: e.target.value })
    }
        ...
        <React.Fragment>
            <div className="page-content">
                <Container fluid={true}>
                    <Breadcrumbs title="Edit Properties" breadcrumbItem="Properties" />
                                        ...
                                        <Form className="form-horizontal">
                                            <Card>
                                                <CardBody>
                                                    <h4 className="mb-3 text-primary">
                                                        {" "}
                                                        Edit Properties - {editPropertyState.reference || ''}
                                                    </h4>
                                                    <div
                                                        className="my-3"
                                                        style={{
                                                        ...
                                                        <Row>
                                                            ...
                                                            <Col md={8}>
                                                                <Field
                                                                    ...
                                                                    value={editPropertyState.reference}
                                                                    className={
                                                                        "form-control" +
                                            ...
                                            <Row>
                                                <Col md={8} className="mt-1 d-flex justify-content-end">
                                                    <button className="btn btn-primary w-md" type="submit" onClick={goBack}
                                                    >
                                                        <i className="fas fa-times me-1"></i> Go Back
                                                    </button>
                                                    <button
                                                        className="btn btn-primary w-md ms-2"
                                                        type="submit"
                                                        // disabled={addressState.suburb && addressState.street ? false : true}
...
const mapStateToProps = gstate => {
    ...
    const { property_update_listing_info_loading } = gstate.Listing
    return {
        property_list_data,
        property_list_error,
        property_list_loading,
        ...
        property_type_data,
        property_type_error,
        property_type_loading, property_update_listing_info_loading
    };
};
...
export default withRouter(
    connect(mapStateToProps, {
        ...
        propertyUpdateFresh,
        getPropertyInfo,
        getPropertyEditInfoFresh, updatePropertyInfoFromListingList, updatePropertyInfoFromListingListFresh, ListingListInfo
