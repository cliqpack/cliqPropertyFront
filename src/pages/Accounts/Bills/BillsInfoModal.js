...
import {
  ...
  receipt_folio_balance,
  receipt_folio_balance_fresh,
} from "store/actions";
...
const BillsInfoModal = props => {
  ...
  const payHandler = () => {
    ...
    props.PendingBillsForOwner(
      props.propertyId ? props.propertyId : "",
      billData.owner_folio_id ? billData.owner_folio_id : ""
    );
  };
  const deleteHandler = () => {
      ...
      <Modal isOpen={props.showModal} toggle={props.toggleModal} size="lg">
        <ModalHeader toggle={props.toggleModal}>
          <span className="text-primary">
            Bill #{billData?.id || ""} - {billData?.invoice_ref}
          </span>
        </ModalHeader>
        ...
        <ModalBody>
              ...
              <div className="button-items">
                {props.data?.status === "Unpaid" &&
                  props.data?.approved === 1 && (
                    <button
                      ...
                      onClick={payHandler}
                      disabled={
                        folioBalance?.balance >= billData?.amount ? false : true
                      }
                    >
                      Pay
                    </button>
                ...
                {props.data?.approved === 1 && (
                  <button
                    type="button"
                    className="btn btn-info me-1"
                    onClick={props.billEditModalToggle}
                  >
                    Edit & Replace
                  </button>
                )}
            ...
            <Col md={5}>
              <div className="d-flex justify-content-end align-items-center">
                ...
                >
                  <span>Folio</span>
                  <span>
                    {billData?.owner_folio_id != null
                      ? billData?.owner?.owner_folio?.folio_code
                      : billData?.sellers?.folio_code}
                  </span>
                </div>
                <div
                  style={{
                  ...
                  className={`d-flex flex-column justify-content-between align-items-center p-2 ${
                    folioBalance?.balance >= billData?.amount
                      ? "text-success"
                      : "text-danger"
                  }`}
                >
                ...
                <div className="py-2">
                  <Row>
                    ...
                    <Col className="d-flex flex-column">
                      ...
                      <div>
                        <Link
                          to={`contactsInfo/${billData?.supplier?.contact_id}`}
                        >
                          {billData?.supplier?.reference}
                        </Link>
                      </div>
                    </Col>
                    {/* <Col className="d-flex flex-column">
                    ...
                    <Col className="d-flex flex-column">
                      ...
                      <div>
                        {billData?.receipt_id && `#${billData?.receipt_id}`}{" "}
                      </div>
                    </Col>
                  </Row>
                </div>
                <div className="py-2">
                  <Row>
                    <Col className="d-flex flex-column">
                      ...
                      <div>
                        {moment(billData?.billing_date).format("YYYY-MM-DD")}
                      </div>
                    </Col>
                    <Col className="d-flex flex-column">
          ...
          <div className="mx-1 mt-2 border">
            <div className="mb-3 ps-2 py-2 border-bottom fw-bold d-flex justify-content-start align-items-center">
              Bill Details
            </div>
                ...
                <tbody>
                  <tr>
                    ...
                    <td>
                      {billData?.file && (
                        <a
                          href={`${process.env.REACT_APP_IMAGE}${billData?.file}`}
                          rel="noreferrer"
                          target="_blank"
                        >
                          <i className="fas fa-paperclip" />
                ...
                <Table className="table mb-2">
                  ...
                  <tbody>
                    <tr>
                      <td>
                        {
                          billData?.receipt?.receipt_details?.[1]
                            ?.contact_reference?.reference
                        }
                      </td>
                      <td>
                        {billData?.receipt?.receipt_details?.[1]?.folioCode}
                      </td>
                      <td>
                        {
                          billData?.receipt?.receipt_details?.[1]?.account
                            ?.account_name
                        }
                      </td>
                      <td>
                        {billData?.receipt?.receipt_details?.[1]?.description}
                      </td>
                      <td>
                        {billData?.receipt?.receipt_details?.[1]?.tax === 1 ? (
                          <i className="fas fa-check" />
                        ) : (
                          ""
                        )}
...
const mapStateToProps = gstate => {
  ...
  const { pay_bill_loading, delete_bill_loading, approve_bill_loading } =
    gstate.Bills;
