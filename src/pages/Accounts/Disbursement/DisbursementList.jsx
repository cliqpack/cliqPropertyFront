...
import {
  dueDisbursementList,
  dueDisbursementListFresh,
  previewDisbursement,
  previewDisbursementFresh,
  storeDisbursement,
  storeDisbursementFresh,
  storeSupplierDisbursement,
  storeSupplierDisbursementFresh,
} from "store/actions";
...
function DisbursementList(props) {
  ...
  const [allSupplierDisbursementData, setAllSupplierDisbursementData] =
    useState([]);
  ...
  const toggle = (tab, type) => {
    ...
    if (type === "DueOwners") {
      ...
      props.dueDisbursementList(
        type,
        dataState.page,
        dataState.sizePerPage,
        null,
        "next_disburse_date",
        "desc"
      );
  ...
  const totalPayout = row => {
    ...
    let invoices = row.total_due_invoice_sum_amount
      ? row.total_due_invoice_sum_amount
      : 0;
    let rent = row.total_due_rent_sum_amount
      ? row.total_due_rent_sum_amount
      : 0;
    let bills = row.total_bills_amount_sum_amount
      ? row.total_bills_amount_sum_amount
      : 0;
    ...
    let deposit = row.total_deposit_sum_amount
      ? row.total_deposit_sum_amount
      : 0;
  ...
  const supplierBillPending = (cell, row) => {
    return (
      <span>
        {row.total_bills_pending_sum_amount
          ? "$" + row.total_bills_pending_sum_amount
          : "$0"}
      </span>
    );
    ...
    return (
      <span>
        {row.total_due_invoice_sum_amount
          ? "$" +
            (+row.total_due_invoice_sum_amount -
              +row.total_due_invoice_sum_paid)
          : "$0"}
      </span>
    );
  };
  ...
  const selectRow1 = {
    ...
    onSelect: (row, isSelect, rowIndex, e) => {
      ...
      if (isSelect) {
        ...
        setState({
          ...state,
          DUEOWNselected: [...state.DUEOWNselected, row.id],
        });
      } else {
        ...
        setState({
          ...state,
          DUEOWNselected: state.DUEOWNselected.filter(data => data !== row.id),
        });
      }
    },
  ...
  const selectRow2 = {
    ...
    onSelect: (row, isSelect, rowIndex, e) => {
      ...
      if (isSelect) {
        ...
        setState({
          ...state,
          ALLOWNselected: [...state.ALLOWNselected, row.id],
        });
      } else {
        let index = value.indexOf(row);
        value.splice(index, 1);
        ...
        setState({
          ...state,
          ALLOWNselected: state.ALLOWNselected.filter(data => data !== row.id),
        });
      }
    },
    onSelectAll: (isSelect, rows, e) => {
  ...
  const selectRow3 = {
    ...
    onSelect: (row, isSelect, rowIndex, e) => {
      ...
      if (isSelect) {
        ...
        setState({
          ...state,
          ALLSUPselected: [...state.ALLSUPselected, row.id],
        });
      } else {
        let index = value.indexOf(row);
        value.splice(index, 1);
        ...
        setState({
          ...state,
          ALLSUPselected: state.ALLSUPselected.filter(data => data !== row.id),
        });
      }
    },
    onSelectAll: (isSelect, rows, e) => {
  ...
  const pushToWithdrawal = () => {
    history.push(
      `/withdrawalsList/${moment().format("MM")}/${moment().format("YYYY")}`
    );
  };
                      ...
                      <Row>
                        <div className="d-flex justify-content-between">
                          <div className="d-flex justify-content-start">
                            <button
                              type="button"
                              className="btn btn-md btn-info mt-5 mb-3"
                              onClick={handleDueOwnerSelectedData}
                              disabled={
                                disbursementData.length > 0 ? false : true
                              }
                            >
                              <i className="fas fa-upload me-1"></i> Disburse
                            </button>
                            ...
                            >
                              ...
                              <Input
                                ...
                                checked={
                                  includeSupplier === true ? true : false
                                }
                                onClick={() =>
                                  setIncludeSupplier(prev => !prev)
                                }
                              />
                            </div>
                          </div>
                        ...
                        <RemotePagination
                          data={
                            dataState?.data.length > 0 ? dataState.data : []
                          }
                          page={dataState.page}
                    ...
                    <TabPane tabId="2">
                      <Row>
                        <div className="d-flex justify-content-between">
                          <button
                            type="button"
                            className="btn btn-md btn-info mt-5 mb-2"
                            onClick={handleAllOwnerSelectedData}
                            disabled={
                              allOwnerDisbursementData.length > 0 ? false : true
                            }
                          >
                            <i className="fas fa-upload me-1"></i> Disburse
                          </button>
                        ...
                        <div className="d-flex justify-content-between">
                          <div style={{ marginTop: "50px" }}>
                            <button
                              ...
                              onClick={handleAllSupplierSelectedData}
                              disabled={
                                allSupplierDisbursementData.length > 0
                                  ? false
                                  : true
                              }
                            >
