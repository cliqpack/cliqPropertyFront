...
function BankingReceiptList(props) {
  ...
  const pageOptions = {
    sizePerPage: 10,
    totalSize: props.outstanding_list
      ? props.outstanding_list?.data?.length
  ...
  const cashRef = (cell, row) => {
    return (
      <span>
        {row?.receipt?.payment_method === "cash" ? (
          <>${row?.receipt?.amount}</>
        ) : (
          ""
        )}
  ...
  const cardRef = (cell, row) => {
    return (
      <span>
        {row?.receipt?.payment_method === "card" ? (
          <>${row?.receipt?.amount}</>
        ) : (
          ""
        )}
  ...
  const chequeRef = (cell, row) => {
    return (
      <span>
        {row?.receipt?.payment_method === "cheque" ? (
          <>${row?.receipt?.amount}</>
        ) : (
          ""
        )}
  ...
  const propertyRef = (cell, row) => {
    return (
      <span
        ...
        onClick={() => pushToProperty(row?.receipt?.property?.id)}
      >
        {row?.receipt?.property?.reference}
      </span>
    );
  };
    ...
    if (row?.receipt?.payment_method === "Cash") {
      return (
        <span>
          ...
          {row?.receipt?.payment_method}
        </span>
      );
    } else if (row?.receipt?.payment_method === "Card") {
      return (
        <span>
          <i className="far fa-credit-card"></i> {row?.receipt?.payment_method}
        </span>
      );
    } else {
      return (
