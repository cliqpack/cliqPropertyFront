...
import {
  BankList,
  DepositOutstanding,
  DepositOutstandingList,
  DepositOutstandingListFresh,
  DepositOutstandingShow,
  DepositOutstandingShowFresh,
} from "store/actions";
...
function BankingList(props) {
  ...
  useEffect(() => {
    ...
    props.deposit_outstanding_show_loading,
    props.deposit_outstanding_list_loading,
    props.banking_list_loading,
  ...
  const monthRef = (cell, row) => {
    return (
      <span
        ...
        onClick={() => pushToBankDepositedList(row.id)}
      >
        {moment(row.deposit_date).format("MMMM YYYY")}
      </span>
    );
  };
            ...
            <>
              {item.payment_method === "cash" && (
                <Col
                  key={item.id}
                  md={3}
                  onClick={() => pushToBankReceiptList(item.month, item.year)}
            ...
            <Col>
              <div className="button-items">
                <Button
                  color="info"
                  className="btn"
                  onClick={handleDepositAllOutstanding}
                  disabled={
                    props.deposit_outstanding_show?.data?.length > 0
                      ? false
                      : true
                  }
                >
                ...
                >
                  <DropdownToggle
                    ...
                    color="secondary"
                    disabled={
                      props.banking_list?.data?.length > 0 ? false : true
                    }
                  >
                    Actions <i className="mdi mdi-chevron-down"></i>
                  </DropdownToggle>
                  <DropdownMenu>
                    <DropdownItem onClick={toggleCancelModal}>
                      Cancel last deposit
                    </DropdownItem>
                  </DropdownMenu>
                </ButtonDropdown>
