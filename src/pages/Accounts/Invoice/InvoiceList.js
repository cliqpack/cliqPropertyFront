...
function InvoiceList(props) {
  ...
  const amountFormatter = (cell, row) => (
    <span>${row.amount ? row.amount : 0}</span>
  );
  ...
  const docPathFormatter = (cell, row) => {
    return (
      <a
        href={process.env.REACT_APP_DOCUMENT + row.doc_path}
        target="_blank"
        rel="noreferrer noopener"
      >
    ...
    <div className="page-content">
      ...
      {showModalInvoice && (
        <ShowInvoiceModal
          ...
          data={modalDataItem}
          toggleEditInvoiceModal={toggleEditInvoiceModal}
...
const mapStateToProps = gstate => {
  ...
  const { add_tenant_receipt_loading, tenant_deposit_receipt_loading } =
