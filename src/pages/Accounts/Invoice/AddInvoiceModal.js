...
import {
  ...
  editBillsWithTenantInvoice,
  editBillsWithTenantInvoiceFresh,
  futureInvoiceBillList,
  invoiceChartList,
  invoiceDueList,
  invoiceTenant,
  invPropertyList,
...
const AddInvoiceModal = props => {
  ...
  const [optionGroupSupplier, setOptionGroupSupplier] = useState([
    {
      options: [{ label: "Mytown Properties", value: "Mytown Properties" }],
    },
  ]);
  ...
  if (
    ...
    setSelectedInvoiceChart({
      label: props.data?.chart_of_account?.account_name,
      value: props.data?.chart_of_account_id,
    });
    setSelectedProperty({
      label: props.data?.property?.reference,
      value: props.data?.property_id,
    });
  ...
  const handleSave = e => {
    ...
    if (!state?.totalInvoiceAmount) {
      ...
      state?.totalInvoiceAmount > 0 &&
      state?.allocatedAmount > state?.tenantDepositAmount
    ) {
      setErrorMessage(
        "The allocated amount cannot be more than the total invoice nor the available tenant deposit amount."
      );
      ...
      state?.allocatedAmount > 0 &&
      state?.totalInvoiceAmount > 0 &&
      state?.allocatedAmount > state?.totalInvoiceAmount
    ) {
      setErrorMessage(
        "The allocated amount cannot be more than the total invoice nor the available tenant deposit amount."
      );
  ...
  useEffect(() => {
    ...
    if (props.tenancy_list_data?.data) {
      optionTen = {
        label: props.tenancy_list_data?.data.reference,
        value: props.tenancy_list_data?.data.id,
      };
  ...
  useEffect(() => {
    if (props.tenancy_list_loading === "Success") {
      ...
      setState(prev => ({
        ...prev,
        tenancy: props.tenancy_list_data?.data?.id,
        tenant_folio_id: props.tenancy_list_data?.data?.tenant_folio.id,
        tenantDepositAmount: props.tenancy_list_data?.data?.tenant_folio
          ?.deposit
      ...
      >
        ...
        >
          <span className="text-primary">
            {props.data ? "Edit Invoice" : "New Invoice"}
          </span>
        </ModalHeader>
        <ModalBody>
          <Card>
                ...
                <Col sm={6} md={6}>
                  {
                    props.data ? (
                      <div style={{ height: "100%" }}>
                        <iframe
                          src={
                            process.env.REACT_APP_IMAGE + props.data?.file ||
                            dummyImage
                          }
                          height="100%"
                          style={{
                    ...
                    <Col md={6}>
                      <div className="btn-group btn-group-justified">
                        <div className="btn-group">
                          <Button
                            color={includeBtn ? "secondary" : "light"}
                            onClick={toggleIncludeBtn}
                            className="btn w-md"
                          >
                            {includeBtn ? (
                        ...
                        <div className="btn-group">
                          <Button
                            color={excludeBtn ? "secondary" : "light"}
                            onClick={toggleExcludeBtn}
                            className="btn w-md"
                          >
                            {excludeBtn ? (
                              <i className="bx bx-comment-check"></i>
                    ...
                    <>
                      <Card>
                        <CardBody className="border-3 border-start border-danger">
                          ...
                          <p>
                            Would you like to allocate an amount towards this
                            invoice? If yes, please fill in the allocated
                            amount.
                          </p>
...
const mapStateToProps = gstate => {
  const { inv_property_list_data, inv_property_list_loading } = gstate.property;
  const { edit_invoice_loading } = gstate.AccountsInvoices;
