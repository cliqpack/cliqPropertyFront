...
import AddReceipt from "../Accounts/Transactions/AddReceipt"
...
const PropertyInfo = props => {
    ...
    const handleTenantFiles = (e, propId, contactId, tenantId, access) => {
        e.preventDefault();
        props.storePropertyDocument(e.target.files, propId, contactId, tenantId, null, null, null, access);
    };
    ...
    const handleOwnerFiles = (e, propId, contactId, ownerId, access) => {
        e.preventDefault();
        props.storePropertyDocument(
            e.target.files,
            propId,
    ...
    console.log(props.property_info_data);
    ...
    const handleUndoArchive = () => {
        ...
        props.SaleAgreementInfoFresh();
        props.getPropertyTenantInfo(id)
    };
    ...
    const togglePhoneModal = (phoneNumber) => {
        setIsPhoneModalOpen(!isPhoneModalOpen);
        if (phoneNumber) {
            setSMSTempEditData((prevState) => ({
                ...prevState,
                to: phoneNumber,
            }));
        }
    ...
    const toggleEmailModal = (contactData) => {
        const fullName = `${contactData.first_name} ${contactData.last_name}`;
        const formattedContact = {
            ...contactData,
            full_name: fullName,
            display: fullName,
            label: `${fullName} (${contactData.email})`,
    ...
    // Sample contact data (to replace external contacts prop)
    ...
    useEffect(() => {
        if (!isContactLoading && contacts.length > 0 && !emailState.selectedContacts) {
    ...
    // Function for handling CC input changes
    const handleCCInputChange = (ccEmails) => {
        setSelectedCC(ccEmails);
    };

    // Function for handling BCC input changes
    const handleBCCInputChange = (bccEmails) => {
        setSelectedBCC(bccEmails);
    };
    ...
    // Function for handling file attachment input
    const handleAttachment = async (e) => {
        ...
        const Url = `${process.env.REACT_APP_LOCALHOST}/mail/attachment`;
    ...
    // Function for handling contact selection
    const selectHandlerForMessage = (selectedOption) => {
        setEmailState(prevState => ({
            ...prevState,
    ...
    // Function for handling email body input from CKEditor
    const handleEditorChange = (event, editor) => {
        const data = editor.getData();
        setEmailState(prevState => ({ ...prevState, body: data }));
    };

    // Mock function for email submission (replace with your logic)
    const handleEmailSubmit = async () => {
        ...
        const Url = `${process.env.REACT_APP_LOCALHOST}/message/mail`;

        const data = {
            mail_id: updatedEmailState.id,
            to: updatedEmailState.to,
            from: authUser.user.email,
            subject: updatedEmailState.subject,
            body: updatedEmailState.body,
            attached: attached,
            cc: selectedCC,
            bcc: selectedBCC,
            status: "outbox",
        ...
        try {
            ...
            setLoader(false)
            setIsEmailModalOpen(!isEmailModalOpen);
            setEmailState({ ...emailState, body: "", subject: "" });
            setAttached([])
            setSelectedBCC([])
            setSelectedCC([])
            setOpenCC(false)
            ...
            const errorMessage = error.response?.data?.message || error.message || "An error occurred while archiving the property.";
            toastr.warning(errorMessage);
            setLoader(false)
        }
    };

    // State to manage SMS data
    ...
    const handleSubmitSMS = async () => {
        ...
        const Url = `${process.env.REACT_APP_LOCALHOST}/message-sms`;
        const data = {
            to: smsTempEditData.to,
            from: "noreply@myday.com",
            body: smsTempEditData.message,
            type: "sms",
        };
    ...
    const [dropdownOpenOwner, setDropdownOpenOwner] = useState(false);
    const toggleDropdownOwner = () => setDropdownOpenOwner(!dropdownOpenOwner);

    const onFileChange = (e, clientAccess) => {
        handleTenantFiles(
            e,
            id,
    ...
    const onFileChangeOwner = (e, clientAccess) => {
        handleOwnerFiles(
            e,
            id,
    ...
    const triggerFileUpload = (clientAccess) => {
        inputFileTenant.current.click();
        inputFileTenant.current.onchange = (e) => onFileChange(e, clientAccess);
    };

    const triggerFileUploadOwner = (clientAccess) => {
        inputFileOwner.current.click();
        inputFileOwner.current.onchange = (e) => onFileChangeOwner(e, clientAccess);
    };
    ...
    const handleArchiveSave = async () => {
        const formattedAgreementEndDate = formatDateForApi(agreementEndDate);
        ...
        const data = {
            agreement_end: formattedAgreementEndDate,
    ...
    const fetchLostReasons = async () => {
        ...
        const Url = `${process.env.REACT_APP_LOCALHOST}/reason/setting`;

        try {
            const response = await axios.get(Url, { headers });
            const reasons = response.data.data.filter(item => item.type === "Lost");
            setLostReasonList(reasons);
        } catch (err) {
        }
    };
        ...
        <React.Fragment>
                                                        ...
                                                        >
                                                            ...
                                                            <DropdownMenu>
                                                                ...
                                                                <Link to={`/manageOwner/${id}`}>
                                                                    <DropdownItem>View Owner</DropdownItem>
                                                                </Link>
                                                            ...
                                                            <DropdownMenu>
                                                                ...
                                                                {property_data?.data?.status == "Active" && (
                                                                    <>
                                                                        ...
                                                                        <DropdownItem onClick={() => setArchiveModalOpen(true)}>
                                                                            Archive
                                                                        </DropdownItem>
                                                                    </>
                                                                )}
                                        ...
                                        <CardBody>
                                            ...
                                            >
                                                <Row className="d-flex justify-content-between">
                                                    ...
                                                    {user_type === "Property Manager" && property_data?.data?.status !== "Archived" && (
                                                        <Col
                                                            style={{ cursor: "pointer" }}
                                                            md={6}
                                                            className="d-flex justify-content-end"
                                                        >
                                                            <>
                                                                <Dropdown isOpen={dropdownOpen} toggle={toggleDropdown}>
                                                                    <DropdownToggle color="labelColor" caret>
                                                                        <i className="fas fa-cloud-upload-alt font-size-16 text-white"></i>
                                                                    </DropdownToggle>
                                                                    <DropdownMenu>
                                                                        <DropdownItem onClick={() => triggerFileUpload(0)}>
                                                                            Upload and hide in client access
                                                                        </DropdownItem>
                                                                        <DropdownItem onClick={() => triggerFileUpload(1)}>
                                                                            Upload and show in client access
                                                                        </DropdownItem>
                                                                    </DropdownMenu>
                                                                </Dropdown>
                                                                <input
                                                            ...
                                                            {tenantInfoData?.data?.mobile_phone ? (
                                                                <Col md={6}>
                                                                    <Row className="py-1">
                                                                        ...
                                                                        <Col md={7}>
                                                                            <span
                                                                                onClick={() => togglePhoneModal(tenantInfoData?.data?.mobile_phone)}
                                                                        ...
                                                                        <Col md={7}>
                                                                            <span
                                                                                ...
                                                                                onClick={() => toggleEmailModal({
                                                                                    id: tenantInfoData?.data?.id,
                                                                                    company_id: tenantInfoData?.data?.company_id,
                                                                                    display: tenantInfoData?.data?.full_name,
                                                                                    email: tenantInfoData?.data?.email,
                                                                                    first_name: tenantInfoData?.data?.first_name,
                                                                                    last_name: tenantInfoData?.data?.last_name,
                                                                                })}
                                                                            >
                                                                                {tenantInfoData?.data?.email || ""}
                                                                            </span>
                                                ...
                                                <div className="">
                                                    <Row className="d-flex justify-content-between">
                                                        ...
                                                        {user_type == "Property Manager" && (
                                                            ...
                                                            >
                                                                <>
                                                                    <Dropdown isOpen={dropdownOpenOwner} toggle={toggleDropdownOwner}>
                                                                        <DropdownToggle color="labelColor" caret>
                                                                            <i className="fas fa-cloud-upload-alt font-size-16 text-white"></i>
                                                                        </DropdownToggle>
                                                                        <DropdownMenu>
                                                                            <DropdownItem onClick={() => triggerFileUploadOwner(0)}>
                                                                                Upload and hide in client access
                                                                            </DropdownItem>
                                                                            <DropdownItem onClick={() => triggerFileUploadOwner(1)}>
                                                                                Upload and show in client access
                                                                            </DropdownItem>
                                                                        </DropdownMenu>
                                                                    </Dropdown>
                                                                            ...
                                                                            <Row>
                                                                                ...
                                                                                <Col md={7}>
                                                                                    <span style={{ cursor: 'pointer', color: '#007bff' }}>
                                                                                        {property_data?.data?.current_owner?.mobile_phone && (
                                                                                            <span onClick={() => togglePhoneModal(property_data.data.current_owner.mobile_phone)}>
                                                                                                (m) {property_data.data.current_owner.mobile_phone}
                                                                                            </span>
                                                                                        )}
                                                                                        {property_data?.data?.current_owner?.home_phone && (
                                                                                            <span onClick={() => togglePhoneModal(property_data.data.current_owner.home_phone)}>
                                                                                                (h) {property_data.data.current_owner.home_phone}
                                                                                            </span>
                                                                                        )}
                                                                                        {property_data?.data?.current_owner?.work_phone && (
                                                                                            <span onClick={() => togglePhoneModal(property_data.data.current_owner.work_phone)}>
                                                                                                (w) {property_data.data.current_owner.work_phone}
                                                                                            </span>
                                                                                        )}
                                                                                    </span>
                                                                                </Col>
                                                                                    ...
                                                                                    <Row>
                                                                                        ...
                                                                                        <Col md={8}>
                                                                                            <span
                                                                                                style={{ color: "#007bff", cursor: "pointer" }}
                                                                                                onClick={() => toggleEmailModal({
                                                                                                    id: property_data?.data?.current_owner?.id,
                                                                                                    company_id: property_data?.data?.current_owner?.company_id,
                                                                                                    display: property_data?.data?.current_owner?.full_name,
                                                                                                    email: property_data?.data?.current_owner?.email,
                                                                                                    first_name: property_data?.data?.current_owner?.first_name,
                                                                                                    last_name: property_data?.data?.current_owner?.last_name,
                                                                                                })}
                                                                                            >
                                                                                                {property_data?.data?.current_owner?.email || ""}
                                                                                            </span>
                                                                                        </Col>
                                                                                    </Row>
                                                                                    <div
            ...
            >
                <div className="modal-content">
                        ...
                        <form>
                            <div className="mb-3">
                                <Row>
                                    ...
                                    >
                                        <Select
                                            value={emailState.selectedContacts}
                                            options={contacts}
                            ...
                            <CKEditor
                                ...
                                config={{ placeholder: "Write your email..." }}
                                data={""}
                                style={{ zIndex: "1" }}
                                onReady={(editor) => {
                                ...
                                attached.map((item, key) => (
                                    <div key={key} className="bg-info mb-2 p-1">
                                        <span className="text-light">
                                            {key + 1}. {item.name} ({Math.floor(Number(item.file_size) / 1024)} kb)
                                        </span>
                                    </div>
                                ))}
                        </div>
                    ...
                    <ModalFooter>
                        <Button type="button" color="danger" onClick={toggleEmailModal} className="me-1">
                            <i className="fas fa-times me-1"></i> Close
                        </Button>
                        <Button type="button" color="primary" onClick={handleEmailSubmit} className="ms-1">
                        ...
                        <form>
                            ...
                            <div className="mb-3">
                                ...
                                <Input
                                    ...
                                    value={smsTempEditData.to} // Controlled input
                                />
                            </div>
                            {/* Row for "Message" field */}
                            <div>
                                ...
                                <Input
                                    ...
                                    name="textarea"
                                    className="form-control"
                                    value={smsTempEditData.message} // Controlled input
                                    onChange={e =>
                                        setSMSTempEditData({
                                            ...smsTempEditData,
            ...
            <Modal isOpen={archiveModalOpen} toggle={() => setArchiveModalOpen(false)} className="modal-lg">
                <div className="modal-header flex-column">
                    <h5 className="modal-title mb-2" id="archiveModalLabel">
                        Are you sure you want to archive {property_data?.data?.reference}?
                    </h5>
                    <p className="text-muted">
                        This will close all the owner, tenant, and sale folios linked to this property from future transactions and make the property read-only. All active reminders will be removed.
                    </p>
                    <button
                        type="button"
                ...
                <div className="modal-body">
                    <Row>
                        <Col md={12}>
                            ...
                            <Row className="mb-3">
                                <Col md={3}>
                                    <Label className="form-label text-dark">Agreement End</Label>
                                </Col>
                                <Col md={9}>
                                    <Flatpickr
                                        className="form-control"
                            ...
                            <Row className="mb-3">
                                <Col md={3}>
                                    <Label className="form-label text-dark">Lost Reason</Label>
                                </Col>
                                <Col md={9}>
                                    <select
                                        className="form-control"
            ...
            {documentModal && (
                <PropertyDocs
                    ...
                    id={id}
                    getAllPropertyDocs={props.AllPropertyDocument}
                />
            )}
        </React.Fragment>
    );
