...
import { sellerFolioLedgerReport, storeAttachmentForSendInEmail, FilterSellerFolioLedgerReportApi, FilterSellerFolioLedgerReportApiFresh } from "store/actions";
...
const SellerFolioLedger = props => {
    ...
    useEffect(() => {
        props.sellerFolioLedgerReport(id);
    }, []);
    useEffect(() => {
        if (props.seller_fl_data_loading === 'Success') {
            endLoader()
            props.FilterSellerFolioLedgerReportApiFresh()
        }
    }, [props.seller_fl_data_loading])

    return (
        <React.Fragment>
            <div>
                <InvoiceHeader printDiv={printDiv} downloadPdfDocument={downloadPdfDocument} sendDocumentInEmail={sendDocumentInEmail} />
                <SellerFolioLedgerFilter startLoader={startLoader} id={id}
                    FilterSellerFolioLedgerReportApi={props.FilterSellerFolioLedgerReportApi}
                />
                                    ...
                                    <div>
                                        ...
                                        <div className="table-responsive">
                                            <Table className="table mb-0">
                                                ...
                                                <tbody>
                                                    {
                                                        props.seller_fl_data?.data?.folio_ledger?.length > 0 &&
                                                        <tr>
                                                            <td colSpan="12">
                                                                <span className="fw-bold font-size-16">
                                                                    {props.seller_fl_data?.data?.seller_contacts?.reference} -{" "}
                                                                    {props.seller_fl_data?.data?.folio_code}
                                                                </span>
                                                            </td>
                                                        </tr>
                                                    }
                                                    {props.seller_fl_data?.data?.folio_ledger.map((item2, index2) => {
...
const mapStateToProps = gstate => {
    const { seller_fl_data, seller_fl_data_loading } = gstate.AccountsTransactions;
    const { mail_attachment_data } = gstate.Message;
    return {
        seller_fl_data, seller_fl_data_loading, mail_attachment_data
    };
};

export default withRouter(
    connect(mapStateToProps, {
        sellerFolioLedgerReport, storeAttachmentForSendInEmail, FilterSellerFolioLedgerReportApi, FilterSellerFolioLedgerReportApiFresh
    })(SellerFolioLedger)
);
