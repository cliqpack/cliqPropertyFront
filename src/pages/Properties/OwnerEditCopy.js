...
const OwnerEdit = props => {
  ...
  useEffect(() => {
    ...
    if (props.owner_info_loading == "Success") {
      ...
      setState2({
        ...state2,
        id: props.owner_info_data?.data?.data?.owner_folio?.id,
        total_money:
      ...
      setDateCheck({
        ...dateCheck,
        nextDisburseDate: props.owner_info_data?.data?.data?.owner_folio
          ?.next_disburse_date
          ? true
          : false,
      });
      ...
      let folio_fee_data = [];
      if (props.owner_info_data?.data?.data?.owner_fees.length > 0) {
        props.owner_info_data?.data?.data?.owner_fees.map((item, idx) => {
          folio_fee_data.push({
            selectedValues: {
            ...
            notes: item?.notes,
            amount: item?.amount,
            priceType: item?.fee_settings?.value,
          });
        });
      }
      setState3([...folio_fee_data]);
      setRows4([...props.owner_info_data?.data?.data?.owner_property_fees]);
      let property_fee_data = [];
      if (props.owner_info_data?.data?.data?.owner_property_fees.length > 0) {
        props.owner_info_data?.data?.data?.owner_property_fees.map(
          (item, idx) => {
  ...
  const handleChangeInput = async (idx, e, type) => {
    ...
    values[idx]["priceType"] = e.priceType;
    await setState3(values);
                                  ...
                                  >
                                    <table style={{ width: "100%" }}>
                                      <tbody>
                                        ...
                                        >
                                          <CardBody>
                                            <CardTitle className="text-primary mb-4"></CardTitle>
                                            {rows4.map((item, idx) => (
...
const mapStateToProps = gstate => {
  const { get_ownership_fee_data, get_folio_fee_data } = gstate.FeeSettings;
