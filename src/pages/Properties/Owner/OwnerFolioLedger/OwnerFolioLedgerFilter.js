...
import OwnerFilterDynamicReport from "./OwnerFilterDynamicReport";

function OwnerFolioLedgerFilter(props) {
  ...
  let firstDayOfLastMonth = currentDate
    .clone()
    .subtract(1, "month")
    .startOf("month");

  // Get the last day of the previous month
  let lastDayOfLastMonth = currentDate
    .clone()
    .subtract(1, "month")
    .endOf("month");
  ...
  const handleTransactionOn = e => {
    if (e.value === "Today") {
      ...
      setState({
        ...state,
        transactionOn: e,
        from_date: formattedLastMonthFirstDay,
        to_date: formattedLastMonthLastDay,
      });
    } else {
  ...
  const fromDateHandler = (selectedDates, dateStr, instance) => {
    setState({
      ...state,
      ["from_date"]: dateStr,
      transactionOn: { label: "Custom", value: "Custom" },
    });
  };
  ...
  const handleFilterFunction = () => {
    props.startLoader();
    props.FilterOwnerFolioLedgerReportApi(props.id, state);
  };
    ...
    >
      ...
      <Col md={3}>
        <ReactSelectComp
          ...
          value={state.transactionOn}
          options={state.optionsTransactionOn}
          handler={handleTransactionOn}
        />
      </Col>
      <Col md={2}>
...
export default OwnerFolioLedgerFilter;
