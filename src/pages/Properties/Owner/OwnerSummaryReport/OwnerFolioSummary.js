...
import { ownerFolioSummaryTransaction, ownerFolioSummaryTransactionFresh, ownerFolioSummaryProperties } from "store/actions";
...
function OwnerFolioSummary(props) {
    ...
    const handleSummaryTransaction = () => {
        startLoader()
        props.ownerFolioSummaryTransaction(id, state)
    }
    ...
    function handleFinancialSummary () {
        history.push(`/edit/financial/summary/${id}`)
    }
    ...
    useEffect(() => {
        if (props.owner_fs_transaction_loading == 'Success') {
            let incomedata = props.owner_fs_transaction?.income_accounts?.data?.map(item => item.account?.account_name)
            let incomecolor = props.owner_fs_transaction?.income_accounts?.data?.map((item, index) => greenGradient[index])
            let incomedataValue = props.owner_fs_transaction?.income_accounts?.data?.map(item => ({ value: item.total_amount, name: item.account?.account_name }))
            SetPieChartIncomeState(prev => ({ ...prev, data: incomedata, color: incomecolor, dataValue: incomedataValue }))
            let expensedata = props.owner_fs_transaction?.expense_accounts?.data?.map(item => item.account?.account_name)
            let expensecolor = props.owner_fs_transaction?.expense_accounts?.data?.map((item, index) => redGradient[index])
            let expensedataValue = props.owner_fs_transaction?.expense_accounts?.data?.map(item => ({ value: item.total_amount, name: item.account?.account_name }))
            SetPieChartExpenseState(prev => ({ ...prev, data: expensedata, color: expensecolor, dataValue: expensedataValue }))
            endLoader()
            props.ownerFolioSummaryTransactionFresh()
        }
    }, [props.owner_fs_transaction_loading])
                    ...
                    <div className="d-flex justify-content-start align-items-center py-2 px-4">
                        ...
                        <div className="button-items d-flex justify-content-start">
                            ...
                            >
                                ...
                                <DropdownMenu>
                                    <DropdownItem>
                                        <Link style={{ color: 'black' }} to={`/summary/bydate/${id}/${state.from_date}/${state.to_date}`} target='blank'>By Date</Link>
                                    </DropdownItem>
                                    <DropdownItem>
                                        <Link style={{ color: 'black' }} to={`/summary/monthinfo/${id}/${state.from_date}/${state.to_date}`} target='blank'>By Date with Monthly Info</Link>
                                    </DropdownItem>
                                    <DropdownItem>
                                        <Link style={{ color: 'black' }} to={`/summary/bystatement/${id}/${state.from_date}/${state.to_date}`} target='blank'>By Statements</Link>
                                    </DropdownItem>
                                    <DropdownItem>
                                        <Link style={{ color: 'black' }} to={`/summary/groupby/${id}/${state.from_date}/${state.to_date}`} target='blank'>Group by Statements</Link>
                                    </DropdownItem>
                                </DropdownMenu>
                            </Dropdown>
                        </div>
            ...
            <Row className="mb-4">
                ...
                <Col md="4">
                    ...
                    <Row>
                        ...
                        <Col md="4">${ceilToTwoDecimalPlaces(props.owner_fs_transaction?.total_credit_tax_sum)}</Col>
                        <Col md="4">${ceilToTwoDecimalPlaces(props.owner_fs_transaction?.total_credit_amount_sum)}</Col>
                    </Row>
                    <Row>
                        <Col md="4"><span className="text-primary py-2">Debits</span></Col>
                        <Col md="4">${ceilToTwoDecimalPlaces(props.owner_fs_transaction?.total_debit_tax_sum)}</Col>
                        <Col md="4">${ceilToTwoDecimalPlaces(props.owner_fs_transaction?.total_debit_amount_sum)}</Col>
                    </Row>
                </Col>
            </Row>
            <Row>
                        ...
                        >
                                ...
                                <Row>
                                    <Col sm="12">
                                        <CardText className="mb-0">
                                            {props.owner_fs_transaction?.distinct_receipt_details ? (
                                                <DatatableTables2
                                                    products={props.owner_fs_transaction?.distinct_receipt_details}
                            ...
                            <TabPane tabId="2">
                                <Row>
                                    <Col sm="12">
                                        <CardText className="mb-0">
                                            {props.owner_fs_transaction ? (
                                                <DatatableTables2
                                                    products={props.owner_fs_transaction}
...
const mapStateToProps = gstate => {
    const { owner_fs_transaction, owner_fs_transaction_loading, owner_fs_properties } = gstate.AccountsTransactions;
    return {
        owner_fs_transaction, owner_fs_transaction_loading, owner_fs_properties
    };
};

export default withRouter(
    connect(mapStateToProps, {
        ownerFolioSummaryTransaction, ownerFolioSummaryTransactionFresh, ownerFolioSummaryProperties
    })(OwnerFolioSummary)
);