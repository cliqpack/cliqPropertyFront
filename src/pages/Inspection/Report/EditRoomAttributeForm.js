...
const EditRoomAttributeForm = props => {
  ...
  attributeFormInput = props.attribute.map((item, idx) => {
    btnPushState.push({
      clean: { state: item.clean },
      undamaged: { state: item.undamaged },
      working: { state: item.working },
    });
      ...
      <Form>
        ...
        <div style={{ height: "420px" }}>
          <Row className="mb-3">
            ...
            <Col sm={8} style={{ border: "1px dashed #c9c8c3", borderRadius: "2px" }} onDragOver={propertyPhotoDrag}
              onDragLeave={propertyPhotoDragend}
              onDrop={propertyPhotoDropFile}>
              {!showDropZone.propertyPhoto ? <Row className="mb-2">
                <Col md={12}>
                  {slides.length > 0 ? (
                    <Carousel
                  ...
                  <div style={{ marginTop: "-10px", zIndex: 100, height: "30px", width: "100%", textAlign: 'center', position: "absolute", justifyContent: "center", }}>
                    <div style={{ width: "60px", backgroundColor: "#c9c8c3", height: "35px", margin: "auto", padding: "5px", display: "flex", gap: 2, justifyContent: "center", alignItems: "center", borderRadius: "5px" }}><i className="bx bx-image-add" style={{ fontSize: "15px" }} /><span style={{ fontSize: "15px" }}>{itemLength + 1}</span></div>
                  </div>
